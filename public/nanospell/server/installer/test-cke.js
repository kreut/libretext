...
function render() {
	...
	var hasCKE = typeof(CKEDITOR)!=='undefined';
	...
	if(php_all) {
		if(capabilities['dic_php'].length) {
			...
			php_output += '<li>  <a href="http://www.ckeditorspellcheck.com/dictionaries" target="_blank"> Download More &raquo;</a></li>';
			php_output += '</ul> ';
		} else {
			php_output += '<p>NanoSpell Dictionaries Installed: None <i class="glyphicon glyphicon-warning-sign" style="color:red"></i>';
			php_output += '<div class="alert alert-danger" >This is likely to be a file permissions issue, or simply the files have not been copied to the correct directory - which is "nanospell/server/dictionaries" .  You may also want to read ckeditorspellcheck.com/dictionaries </div>';
			php_all = false
		}
	}
	if(php_all) {
	...
	if(php_all) {
		if(hasCKE){$("#php_live").show();
		var path = document.location.pathname + '';
		var filename = path.substring(path.lastIndexOf('/') + 1);
		var url = path.replace(filename, "plugin.js");
		$('.dynamic_url_for_plugin').html(url);
		CKEDITOR.replace('php_live_textarea');
		 nanospell.ckeditor('php_live_textarea',{server:"php"}); 
		
		//$( '#php_live textarea' ).ckeditor({extraPlugins:'nanospell',toolbar:[['Nanospell'],["Cut","Copy","Paste"]]});
	}
	...
	if(asp_all) {
		if(capabilities['dic_asp'].length) {
			...
			asp_output += '<li>  <a href="http://www.ckeditorspellcheck.com/dictionaries" target="_blank"> Download More &raquo;</a></li>';
			asp_output += '</ul> ';
		} else {
			asp_output += '<p>NanoSpell Dictionaries Installed: None <i class="glyphicon glyphicon-warning-sign" style="color:red"></i>';
			asp_output += '<div class="alert alert-danger" >This is likely to be a file permissions issue, or simply the files have not been copied to the correct directory - which is "nanospell/server/dictionaries" .  You may also want to read ckeditorspellcheck.com/dictionaries </div>';
			asp_all = false
		}
	}
	if(asp_all) {
	...
	if(asp_all) {
	if(hasCKE){		$("#asp_live").show();
		var path = document.location.pathname + '';
		var filename = path.substring(path.lastIndexOf('/') + 1);
		var url = path.replace(filename, "plugin.js");
		$('.mydynamic_url_for_plugindir').html(url);
			$( '#asp_live textarea' ).ckeditor({extraPlugins:'nanospell',toolbar:[['Nanospell'],["Cut","Copy","Paste"]],spelling:{server:"asp"} });
		}
	...
	if(capabilities['net']) {
		...
		net_output += '<div class="alert alert-danger" >If ASP.Net is runnig then try copying <b>nanospell/bin/NanoSpell.dll</b> to your /bin folder and re-run this test. </div>';
	...
	if(net_all) {
		if(capabilities['dll']) {
			...
			net_output += '<div class="alert alert-danger" >Copy <b>nanospell/bin/NanoSpell.dll</b> to your /bin folder and re-run this test.  The DLL requires ASP.Net Framework version 2 or above.</div>';
			net_all = false
		}
	}
	if(net_all) {
	...
	if(net_all) {
		if(capabilities['dic_net'].length) {
			...
			net_output += '<li>  <a href="http://www.ckeditorspellcheck.com/dictionaries" target="_blank"> Download More &raquo;</a></li>';
			net_output += '</ul> ';
		} else {
			net_output += '<p>NanoSpell Dictionaries Installed: None <i class="glyphicon glyphicon-warning-sign" style="color:red"></i>';
			net_output += '<div class="alert alert-danger" >This is likely to be a file permissions issue, or simply the files have not been copied to the correct directory - which is "nanospell/server/dictionaries" .  You may also want to read ckeditorspellcheck.com/dictionaries </div>';
			net_all = false
		}
	}
	if(net_all) {
	...
	if(net_all) {
		...
		$('.dynamic_url_for_plugin').html(url);
			...
			//$( '#net_live textarea' ).ckeditor({extraPlugins:'nanospell',toolbar:[['Nanospell'],["Cut","Copy","Paste"]],spelling:{server:".net"}});
			}
