...
class Webwork extends Model
{
    ...
    public function cloneDir($sourceDir, $targetDir)
    {
        ...
        $response = Http::withToken(config('myconfig.webwork_token'))
            ->post("https://{$this->webwork_opl}/api/authored/clone", [
                'source_file_path' => $path . $sourceDir,
                'target_file_path' => $path . $targetDir . '/',
            ]);

        return $response->successful()
            ? 'clone successful'
            : "Error cloning webwork from {$path}{$sourceDir} to {$path}{$targetDir}/";
    }
    ...
    public function storeAttachment($filename, $local_path, $webwork_dir)
    {
        ...
        $response = Http::withToken(config('myconfig.webwork_token'))
            ->attach('file', file_get_contents($local_path), $filename, ['Content-Type' => mime_content_type($local_path)])
