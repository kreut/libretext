...
class SavedQuestionsFoldersController extends Controller
{
    ...
    public
    function destroy(Request $request, SavedQuestionsFolder $savedQuestionsFolder, MyFavorite $savedQuestion): array
    {
        $response['type'] = 'error';
            ...
            switch ($fromFolder->type) {
                ...
                case('my_questions'):
                    DB::table('questions')
                        ...
                        ->where('question_editor_user_id', $request->auth()->id)
                        ->where('folder_id', $fromFolder->id)
                        ->update(['folder_id' => $toFolder->id]);
                    break;
                default:
