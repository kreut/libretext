...
class WebworkMacroController extends Controller
{
     ...
     */
    public function index(Request $request, WebworkMacro $webworkMacro): array
    {
        $response['type'] = 'error';
        try {
    ...
    public function update(StoreWebworkMacroRequest $request, WebworkMacro $webworkMacro): array
    {
        ...
        try {
            ...
            $h = new Handler(app());
            $h->report($e);
            $response['message'] = "We were not able to update the macro. Please try again or contact us for assistance.";
        }
        return $response;
    }
        ...
        try {
            ...
            if ($check_existence) {
                ...
                if ($existing->successful() && count($existing->json()) > 0) {
                    if (!in_array($existing->json()[0]['source_type'], ['custom', 'unknown'])) {
                        return [
                            'success' => false,
