...
class AssignmentTopicController extends Controller
{
                                                    ...
                                                    AssignmentTopic $assignmentTopic): array
    {
        ...
        try {
            $topics_info = DB::table('assignment_topics')
                ->where('assignment_id', $assignment->id)
                ->select('name',
                    'id',
                    'assignment_id')
                ->orderBy('name')
                ->get();
            $assignment_question_counts = DB::table('assignment_question')
                ->where('assignment_id', $assignment->id)
                ->groupBy('assignment_topic_id')
                ->select('assignment_topic_id', DB::raw("COUNT(*) AS num_questions"))
                ->get();
            $assignment_question_counts_by_topic_id = [];
            foreach ($assignment_question_counts as $count){
                $assignment_question_counts_by_topic_id[$count->assignment_topic_id] = $count->num_questions;
            }
            $topics = [];
            foreach ($topics_info as $key => $topic){
                $topics[$key] = $topic;
                $topics[$key]->num_questions = $assignment_question_counts_by_topic_id[$topic->id] ?? 0;
            }
            $response['topics'] = $topics;
            $response['type'] = 'success';
        } catch (Exception $e) {
