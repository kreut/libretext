...
/>
    <span class="text-muted small">sec/side</span>
      ...
      <template #modal-footer>
        <b-button size="sm" @click="$bvModal.hide(`modal-flashcard-student-settings-${uuid}`)">
          {{ initialized ? 'Cancel' : 'Use Defaults' }}
        </b-button>
    ...
    <div class="fc-session">
      ...
      <div class="fc-nav">
        ...
        <div v-if="orderedCards.length > 1 && !isFlipped" class="fc-dots" role="group" aria-label="Card navigation">
          <button
            v-for="(_, i) in dotIndices"
            :key="i"
            type="button"
            class="fc-dot"
            :class="{
              'fc-dot--current': dotIndices[i] === currentIndex,
              'fc-dot--correct': cardResults[dotIndices[i]] === 'correct',
              'fc-dot--incorrect': cardResults[dotIndices[i]] === 'incorrect',
            }"
            :aria-label="`Card ${dotIndices[i] + 1}${cardResults[dotIndices[i]] ? ', ' + cardResults[dotIndices[i]] : ''}`"
            :aria-current="dotIndices[i] === currentIndex ? 'true' : 'false'"
            @click="goTo(dotIndices[i])"
          />
        </div>
      ...
      <transition name="fc-summary-fade">
        ...
        >
          ...
          <div class="fc-summary-actions">
            ...
            <b-button variant="outline-secondary" size="sm" @click="restartDeck">
              <b-icon icon="skip-backward-fill" aria-hidden="true"/>
              Restart
            </b-button>
...
export default {
  ...
  computed: {
    ...
    // Show at most 12 dots; if more cards, show a sliding window around current
    dotIndices () {
      const total = this.orderedCards.length
      const max = 12
  ...
  methods: {
    ...
    onAutoplaySecondsInput (val) {
      ...
      if (isNaN(n)) {
        this.$nextTick(() => { this.studentSettingsForm.autoplay_seconds = 1 })
      } else if (n > 99) {
        this.$nextTick(() => { this.studentSettingsForm.autoplay_seconds = 99 })
      }
    },
    ...
    initialize () {
      ...
      if (this.resolvedSettings.autoplay.enabled && this.user.role === 3) {
        this.toggleAutoplay()
      }
      if (this.resolvedSettings.random_shuffle.enabled && this.user.role === 3) {
        this.shuffleOn = true
        this.applyOrder()
      }
    ...
    applyOrder () {
      ...
      this.$emit('reorderQuestions', this.orderedCards)
    },
