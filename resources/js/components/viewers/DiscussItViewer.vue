<template>
  <div>
    ...
    >
      ...
      <div v-show="activeDiscussionComment.text">
        {{ activeDiscussionComment.text }}
      </div>
    ...
    >
      ...
      <DiscussItSatisfiesRequirement v-if="reRecording"
                                     ...
                                     :time-until-requirement-satisfied="+50000"
                                     :comment-type="'audio'"
    ...
    <b-container style="font-size:small">
      <b-row>
        <b-col cols="8">
          <div v-if="/\.(mp3|mp4)$/.test(currentDiscussionMedia.s3_key)">
            <iframe
              ...
              :src="`/discussion-media-player/assignments/${assignmentId}/question/${questionId}/${currentDiscussionMedia.id}`"
              frameborder="0"
              allowfullscreen
              class="mb-2"
            />
          ...
          <b-pagination
            ...
            :total-rows="qtiJson.mediaUploads.length"
            :per-page="1"
            pills
            align="center"
            first-number
        ...
        <b-col>
          <div class="mb-2">
            <b-card header="default" header-html="<h2 class='h7'>Submission Requirements</h2>">
              <ul v-show="user.role === 3" style="list-style: none">
                <li
...
export default {
  ...
  methods: {
    ...
    showAction (action, userId) {
      ...
      switch (action) {
        case ('editComment'):
          showAction = (this.user.role === 3 && userId === this.user.id && this.discussItSettingsForm.students_can_edit_comments) ||
            this.user.role === 2
          break
        case ('deleteComment'):
          showAction = (this.user.role === 3 && userId === this.user.id && this.discussItSettingsForm.students_can_delete_comments) ||
            this.user.role === 2
          break
      }
      return showAction
    ...
    async saveAudio () {
      if (!document.getElementsByClassName('ar__uploader')[0]) {
        this.$noty.info('Please first record some audio by clicking on the mic icon.')
      } else {
    ...
    updateDiscussions (currentMediaUploadOrder) {
      this.currentDiscussionMedia = this.qtiJson.mediaUploads.find(mediaUpload => mediaUpload.order === currentMediaUploadOrder)
      this.$nextTick(() => {
    ...
    getMediaUploadId () {
      return this.qtiJson.mediaUploads.find(mediaUpload => mediaUpload.order === this.currentMediaUploadOrder).id
    },
