  ...
  methods: {
    ...
    handleFixCKEditor () {
      ...
      if (this.qtiJson.itemBody) {
        ...
        const text = '<p>&nbsp;</p>'
        if (this.qtiJson.itemBody.substring(0, text.length) === text) {
          // replace the first one
          this.qtiJson.itemBody = this.qtiJson.itemBody.replace(text, '')
        }
        if (this.qtiJson.itemBody.slice(-(text.length + 1)).trim() === text) {
          console.log('replacing end')
          // replace the last set of characters
          this.qtiJson.itemBody = this.qtiJson.itemBody.substring(0, this.qtiJson.itemBody.lastIndexOf(text))
        }
        console.log(this.qtiJson.itemBody)
