...
export function getAcceptedFileTypes (fileTypes = '.pdf, .txt, .png, .jpeg, .jpg, .gif, .xlsx') {
  ...
  if (newFile && !oldFile) {
    ...
    if (!this.isStructureImageUploader) {
      ...
      if (this.uploadLevel === 'question') {
        validExtension = this.isOpenEndedAudioSubmission ? /\.(mp3)$/i.test(newFile.name) : /\.(pdf|txt|png|jpeg|jpg|gif|xlsx)$/i.test(newFile.name)
      } else {
